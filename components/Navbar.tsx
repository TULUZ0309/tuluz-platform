...
export function Navbar() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then((res: { data: { user: import('@supabase/supabase-js').User | null } }) => setUser(res.data.user))
  ...
  return (
    <header style={{
      ...
      <nav style={{
        ...
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '.6rem', textDecoration: 'none' }}>
          ...
          <span style={{
            ...
            fontWeight: 700, fontSize: '1.2rem', color: '#2D1E0F',
          }}>Tuluz</span>
        </Link>
        ...
        <div className="nav-desktop" style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          {[
            ...
            <Link key={href} href={href} style={{
              ...
              color: '#6B4A2A', textDecoration: 'none',
              transition: 'color .2s',
            }}
          ...
          {user ? (
            ...
            <>
              <Link href="/login" style={{
                fontSize: '.875rem', fontWeight: 500, color: '#6B4A2A', textDecoration: 'none',
              }}>
                Iniciar sesión
              </Link>
        ...
        <button
          ...
          style={{
            display: 'none',
            padding: '.5rem', borderRadius: '.5rem', background: 'transparent',
            border: 'none', cursor: 'pointer', color: '#C4893A',
          }}
      ...
      {menuOpen && (
        <div style={{
          background: '#FDFAF4', borderTop: '1px solid #EAD9BC',
          ...
          {[
            ...
            <Link key={href} href={href} style={{ color: '#6B4A2A', textDecoration: 'none', fontWeight: 500 }}
          ...
          {user ? (
            <Link href="/dashboard" style={{
              padding: '.7rem', borderRadius: '10px', textAlign: 'center',
              background: '#C4893A', color: '#fff', fontWeight: 600, textDecoration: 'none',
            }} onClick={() => setMenuOpen(false)}>
              Mi Panel
            </Link>
          ) : (
            <>
              <Link href="/login" style={{ color: '#6B4A2A', textDecoration: 'none', fontWeight: 500 }}
                onClick={() => setMenuOpen(false)}>
                Iniciar sesión
              </Link>
              <Link href="/register" style={{
